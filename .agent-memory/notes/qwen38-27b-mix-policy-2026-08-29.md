...
**Production slice 2 (2026-08-30, same identity, `--mode chain`, max-tokens 128):** empty=0 attractor=0 think=0. t1 cached=0 ctx=42; t2 cached=42 ctx=301 (prefix cache from turn 1). Code turn starts `balance = 0` and is not Paris-confused. `runaway=2` is the 128-token cap. Decode 23.1 tok/s both turns (one-session). Artifact: `harness-mq4-pro-prod-chain.json`. Do not unmask.

**Production slice 3 (2026-08-30, DFlash on same isolated home):** `hipfire pull qwen3.8:27b-draft` → `~/.hipfire/models/qwen38-27b-dflash-mq4.hfq` 1.210 GB sha256 `d0a74a232a0e2166d889f823e91e0fbf778d21dd9668d7de055cdecb065401bc`. Serve log: `DFlash draft loaded` (layers=5, hidden=5120, block=8, windowed W=2048). Both turns `drafter=dflash` / JSON `dflash=true`. `--max-think-tokens 1` battery max=128: empty=0 attractor=0 think=0. Factual: Paris / Seine / Eiffel, τ=3.70, 36.9 tok/s, 27 windows. Code: `below_zero` stub + `balance = 0`, τ=9.58, 191.1 tok/s, 12 windows. `runaway=2` is the 128-cap. One-session, not a claim. Artifact: `harness-mq4-pro-prod-dflash.json`. Do not unmask. Do not write `admissions.yml`.

**Production slice 4 (2026-08-30, 8-turn `session_coding.json`, max-tokens 256, same identity):** empty=0 attractor=0 think=0. t7 recall 3/3 (`dedupe`/`hash`/`Result`). t8 retrieval_miss=`dedupe` on both DFlash and AR — the summary says **deduplication**, not the fixture substring; `rayon`+`chunk` present. Harness exit 1 is that wording, not amnesia. DFlash: all 8 turns `drafter=dflash`; τ 1.33–5.07; avg_decode 68.8 tok/s; `qwen-cache resume dflash-jinja` checkpoint stuck at pos=51 until the short haiku stop, then pos=256 (replays hundreds vs cold). AR: avg_decode 31.9 tok/s; cache 0→51→51→911→1872. Artifacts: `harness-mq4-pro-prod-session.json` / `-session-ar.json`. Do not unmask.

**Production slice 5 (2026-08-30, ~4k AR prefill, same identity):** `glimmer_prefill_4096.txt` + `merge_sorted` needle, prompt md5 `b48860d91f3a5362271583f7aa70de7e`. ctx=4434, gen=7, `finish=stop`, recall=1/1 answer `merge_sorted(a, b)`. empty=0 attractor=0 runaway=0. Prefill 700.2 tok/s decode 30.9 tok/s (one-session). Artifact: `harness-mq4-pro-prod-longctx-4k.json`. Production smoke on mq4-pro stops here. Do not unmask. Do not write `admissions.yml`.
