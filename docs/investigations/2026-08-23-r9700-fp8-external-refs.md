...
| **5** | Encoder + `fp8_wmma` dispatch into one prefill GEMM | **Landed (not default-on).** CPU `--format fp8e4m3` qt=40; runtime `DType` + GEMV/GEMM; gfx1201 overwrite-GEMM prefill; Qwen3.5-0.8B encode→load→short serve on R9700. `fp8_wmma` stays off. 0.8B is known-incoherent — not a quality/tok/s/admission claim. | done |
