...
//! - `gemm_f16_wmma_mb8`: fused-transpose F16 vision linear (N=130 → MB8)
//! - `gemm_f16_wmma_mb1_gfx12`: small-N DFlash path (N=8 → MB1 on gfx12)
...
fn test_gemm_f16_wmma_mb1_small_n(gpu: &mut Gpu) {
    // Hits the gfx12 NB=1 dispatch (N<=32) used by DFlash 2 B=8.
    let m = 1024usize;
    let k = 64usize;
    let n = 8usize;
    ...
    let ref_host = gpu.download_f32(&y_ref).unwrap();
    let wmma_host = gpu.download_f32(&y_wmma).unwrap();
    assert_close("gemm_f16_wmma_mb1_n8", &wmma_host, &ref_host, 5.0e-3);
}
...
fn main() {
    ...
    test_gemm_f16_wmma_mb1_small_n(&mut gpu);
