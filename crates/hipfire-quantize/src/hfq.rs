...
impl QuantType {
    ...
    pub(crate) fn from_u8(v: u8) -> Option<Self> {
        match v {
            ...
            // gfx1201 FP8 leaf: warpfront claimed 40/41 for TQ2/BQ1.
            51 => Some(Self::FP8E4M3G256),
            52 => Some(Self::FP8E5M2G256),
            _ => None,
        }
    }
}
...
pub(crate) enum QuantType {
    ...
    // 256 B leaf + 2 B scale = 8.0625 bpw, plus 16 B row header (not llama.cpp's
    // 32-elem / 34 B F8E4M3). Prefill win on gfx1201 is the value; decode is BW-bound.
    FP8E4M3G256 = 51, // OCP E4M3 (4 exp / 3 mantissa) + fp16 g256 scale — quality leaf
    FP8E5M2G256 = 52, // OCP E5M2 (bf8, 5 exp / 2 mantissa) + fp16 g256 scale — range leaf
