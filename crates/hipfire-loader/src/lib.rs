...
pub struct LoadedModel {
    ...
    pub ngram_mod_pool: Option<std::sync::Arc<std::sync::Mutex<hipfire_runtime::ngram_mod::NgramModPool>>>,
    // Shared
...
fn finish_qwen35_load(
    ...
    // Native MTP lives on the bundle and is selected at generate time
    // (`mtp_mode=on` / `HIPFIRE_QWEN_MTP=1`) *before* DFlash. Load the head
    // even when a DFlash draft is present so both routes stay available.
    // Skip when DSpark won (it owns the speculator), adaptive/eviction, or
    // typed mtp=off. On missing/failure: error if forced on, else log/fallback.
    ...
    // Native MTP (`GenerationRoute::QwenMtp`) owns the head on the bundle.
    // Passing it into `build_speculator` would drop it whenever DFlash wins
    // first, and would alias the same GPU tensors with the speculator path.
    bundle.qwen35_mtp_head = mtp;
