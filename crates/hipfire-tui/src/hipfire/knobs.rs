...
pub const KNOBS: &[KnobInfo] = &[
    ...
    KnobInfo {
        ...
        effect: "On = better reasoning on hard prompts, but more tokens/latency. Suppresses DFlash spec-decode; native MTP still runs when selected.",
        default: "on",
        when: "On for hard reasoning/math; off for throughput, code, or to enable DFlash.",
        note: Some("Interacts with dflash_mode: an active thinking budget (max_think_tokens > 0) makes DFlash fall back to plain decode. Native MTP (HIPFIRE_QWEN_MTP / --spec mtp, greedy) still spec-decodes through the think span."),
        options: &[
            ("on", "Emit a hidden think block — stronger reasoning, more tokens. DFlash falls back to AR; native MTP stays eligible."),
